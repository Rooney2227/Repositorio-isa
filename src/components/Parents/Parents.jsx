 ...
 class Parents extends React.Component {
	...
	setPaginationFilters(buscarProy){
		...
		response = this.state.listaParentsBusqueda.map((item, index) => {
			if(	
				...
				(buscarProy == "" || ((item.created_by).toLowerCase()).includes(buscarProy.toLowerCase()))
            )
    ...
    SetHijos(buscarPadre, listahijos){
		...
		response = listahijos.map((item, index) => {
			if(	
				(buscarPadre == "" || String(item.property_category).includes(buscarPadre))
            )
			{
   ...
   getchilds(id_parent){
	...
	.then(response => {
        ...
        this.SetHijos(id_parent, response.results)
   ...
   getParents(){
		...
		.then(response => {
			...
			this.setState({
				listaParents: response.data,
				listaParentsBusqueda: response.data,ErrorParents: true		
			})

			this.setPagination(response.data)

		})
		.catch(error=> console.log("lista parents:"+error), this.setState({ErrorParents:false}))
	...
	this.state.listachilds.map((it) => {
		options.push(
		<tr>									
			   <td className="center-tex">  { it.property_category} </td> 
	           <td className="center-tex" >
							<Link className="label-en-verde" 
                            to={{pathname: "/Detalle", state: {idParent: 0,Name:it.name,status: it.active_record, Seo_friendly:it.seo_friendly, createdate: it.created_at,
                                upd_date:it.updated_at , creador: it.created_by,idChild:it.id,  isHijo: false} }}
                            >
								{it.name}
							</Link>
						</td>
				<td className="center-tex"> {it.seo_friendly} </td>        
		   </tr>
		)
				...
				<div className="container productos-in ficha-tecnica">
					<div class="row align-items-center r-margin-bottom">
						<div className="productos-header-titulo">
							<div className="astra-titulo">Parents</div>
					...
					<div className="container row">
						<div className="col-sm-6 separar-div-m">
                                   <div class="input-group input-text-con-icono-derecha separar-div-m">
										...
										<input className="form-control" value={this.state.buscarProy} onChange={this.onChangebuscarProy} type="text" placeholder="Buscar por name/creador" aria-describedby="lupita" />
					...
					<div className="card-componente-tabla-estados table-responsive">
              				<table>
								...
								<tr>
									...
									<th>Status</th>
                                    <th className="priority-5">Creation date<i class="fa fa-fw fa-sort" onClick={(e) => this.sortObject('created_at')}></i></th>
                                    <th className="priority-5">Update date<i class="fa fa-fw fa-sort" onClick={(e) => this.sortObject('updated_at')}></i></th>
									<th >Created by</th>
									<th></th>
								</tr>
							</thead>
		...
		return	  <React.Fragment>
		 <tr className='card-separad-lin'  key={index} >											
						<td className="center-tex">
						 ...
						 onClick={() =>this.getchilds(item.property_category_id)}  
						 data-toggle="collapse" data-target={"#demo"+item.property_category_id } 
						 aria-hidden="true"> { item.property_category_id}</i>
						 }
						 { this.state.it_parent == item.property_category_id && 
						 <i className="accordion-toggle fa fa-chevron-down" 
						 onClick={() =>this.getchilds(item.property_category_id)}
						 data-toggle="collapse" data-target={"#demo"+item.property_category_id } 
						
                         >{ item.property_category_id} </i> 
                         
						 } 
                       
						 </td> 
						<td className="center-tex" >
							<Link className="label-en-verde" to={{pathname: "/Detalle", state: {idParent: item.property_category_id,Name:item.name,status: item.active_record, Seo_friendly:item.seo_friendly, createdate: item.created_at,
                                                                    upd_date:item.updated_at , creador: item.created_by,idChild:0,  isHijo: false} }} >
								{item.name}
							</Link>
		
						</td>
						<td className="center-tex priority-4"> {item.seo_friendly} </td> 
                        <td className="center-tex"> <div className={item.active_record ==true ? 'label-en-verde-2' : 'label-en-rojo'}> {item.active_record == true ? 'Activo' : 'Inactivo'} </div></td> 
						<td className="priority-5 center-tex">{item.created_at}</td>
                        <td className="priority-5 center-tex">{item.updated_at}</td>
                        <td className="center-tex"><div className="label-en-amarillo"> {item.created_by} </div></td>
		</tr> 
		<tr>
				<td colspan={8} className="hiddenRow">
				<div className="accordian-body collapse" id={"demo"+item.property_category_id } > 
				<div className="card-componente-tabla-estados table-responsive">				
				 <table>
...
const conectParents = connect(mapStateToProps)(Parents);
export { conectParents as Parents };
